...
if (isset($_GET['kd_asset'])) {
  $kd_asset = $_GET['kd_asset'];
}
...
$query = mysqli_query($conn, " SELECT * FROM asset WHERE kode_asset = '$kd_asset'");
...
if ($gambar != '') {
  ...
  if ($tahun_pengadaan == '') {
    $query = "UPDATE `asset` SET kode_asset='$kode_asset',id_kondisi='$id_kondisi',id_status_kepemilikan='$id_status_kepemilikan',
                                     ...
                                     ukuran='$ukuran',asal_usul='$asal_usul',harga='$harga',gambar= '$gambar',
                                     keterangan='$keterangan', merk_type='$merk_type', pemeliharaan='$pemeliharaan', tanggal_pemeliharaan='$tanggal_pemeliharaan' WHERE kode_asset='$kd_asset'";
  } else {
    $query = "UPDATE `asset` SET kode_asset='$kode_asset',id_kondisi='$id_kondisi',id_status_kepemilikan='$id_status_kepemilikan',
                                   id_lokasi='$id_lokasi',id_jenis_asset='$id_jenis_asset', deskripsi_asset='$deskripsi_asset',jumlah='$jumlah', 
  ...
  if ($result) {
    $query1 = "INSERT INTO `history` (`kode_asset`, `deskripsi_history`, `kantor`) 
                            VALUES ('$kode_asset', 'Update', '$kantor')";
    $result1 = mysqli_query($conn, $query1);
    echo '<script type ="text/JavaScript">';
    echo 'alert("DATA BERHASIL DI UPDATE")';
    echo '</script>';
